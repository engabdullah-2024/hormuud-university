    ...
    <main className="min-h-screen bg-muted/30 overflow-x-hidden">
      <section className="w-full px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 lg:gap-10">
          ...
          {/* Tabs + map */}
          <Card className="border-slate-200 bg-background/95 shadow-sm">
            <CardHeader className="border-b bg-slate-50/80 px-4 py-3 sm:px-5 md:px-6">
              <Tabs defaultValue={campuses[0]?.id} className="w-full">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <TabsList className="w-full max-w-full justify-start gap-1 overflow-x-auto rounded-full bg-white p-1 md:w-auto">
                    {campuses.map((campus) => (
                      <TabsTrigger
                        key={campus.id}
                        value={campus.id}
                        className="whitespace-nowrap rounded-full px-3 py-1.5 text-xs md:px-4 md:text-sm"
                      >
                        {campus.label}
                      </TabsTrigger>
                    ))}
                ...
                {campuses.map((campus) => (
                  <TabsContent
                    ...
                    className="pt-4"
                  >
                    <CardContent className="space-y-5 px-4 pb-5 sm:px-5 sm:pb-6 md:px-6">
                      {/* Top info row */}
                      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                        ...
                        <div className="flex w-full flex-col items-start gap-2 sm:w-auto sm:items-end">
                          <div className="relative h-24 w-full overflow-hidden rounded-lg border border-slate-200 bg-slate-100 shadow-sm sm:h-24 sm:w-44 md:h-24">
                            <Image
                              src={campus.image}
                          ...
                          >
                            {/* This opens the map in a new tab; set this to your real Google Maps share link */}
                            <a
